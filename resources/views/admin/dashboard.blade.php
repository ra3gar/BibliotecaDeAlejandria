...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5">
        ...
        <div class="bg-white rounded-xl shadow-sm p-5 flex items-center gap-4">
            ...
            <div>
                <p class="text-2xl font-bold text-gray-800">{{ $totalLoans }}</p>
                <p class="text-sm text-gray-500">Préstamos Totales</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-5 flex items-center gap-4">
            ...
            <div>
                <p class="text-2xl font-bold text-gray-800">{{ $activeLoans }}</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div class="lg:col-span-2 bg-white rounded-xl shadow-sm p-6">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-base font-semibold text-gray-700">Préstamos recientes</h2>
                <a href="{{ route('admin.loans.index') }}" class="text-xs text-amber-600 hover:text-amber-700 font-medium">Ver todos</a>
            </div>
            @if($recentLoans->isEmpty())
                <p class="text-sm text-gray-400 py-4 text-center">No hay préstamos registrados.</p>
            @else
            <div class="divide-y divide-gray-100">
                ...
                <div class="py-3 flex items-center justify-between gap-4">
                    <div class="min-w-0">
                        ...
                        <p class="text-xs text-gray-400">{{ $loan->user->full_name }} — {{ $loan->loan_date->format('d/m/Y') }}</p>
                    </div>
                    @php
                    ...
                    @endphp
                    <span class="flex-shrink-0 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold {{ $colors[$loan->status] ?? 'bg-gray-100 text-gray-600' }}">
    ...
    {{-- Fila 3: Auditoría de cambios en libros --}}
    <div class="bg-white rounded-xl shadow-sm p-6">
        <div class="flex items-center justify-between mb-4">
            <h2 class="text-base font-semibold text-gray-700">Auditoría reciente</h2>
        </div>
        @if($recentAuditLogs->isEmpty())
            <p class="text-sm text-gray-400 text-center py-4">Sin actividad registrada todavía.</p>
        @else
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr>
                        <td class="py-2 pr-4">
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold {{ $actionColor[$log->action] ?? 'bg-gray-100 text-gray-600' }}">
                                {{ $actionLabel[$log->action] ?? $log->action }}
                            </span>
                        </td>
                        <td class="py-2 pr-4 text-gray-700">{{ $log->description }}</td>
                        <td class="py-2 pr-4 text-gray-500">{{ $log->user?->full_name ?? 'Sistema' }}</td>
                        <td class="py-2 text-gray-400 text-xs whitespace-nowrap">{{ $log->created_at->format('d/m/Y H:i') }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
