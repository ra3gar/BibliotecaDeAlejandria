...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 lib-animate">
    ...
    <div class="flex items-center gap-5 mb-8">
        <div class="w-16 h-16 rounded-full bg-mahogany-900 flex items-center justify-center text-gold-400 font-bold text-2xl shrink-0">
            {{ strtoupper(substr($user->first_name, 0, 1)) }}
        </div>
        <div>
            <h1 class="text-xl font-serif font-semibold text-mahogany-900">{{ $user->full_name }}</h1>
            <p class="text-sepia-400 text-sm">{{ $user->email }}</p>
        </div>
    </div>
    ...
    <div x-data="{ tab: 'loans' }">
        ...
        {{-- Tab buttons --}}
        <div class="flex border-b border-parchment-400 mb-6">
            <button @click="tab = 'loans'"
                    :class="tab === 'loans' ? 'border-gold-500 text-gold-600' : 'border-transparent text-sepia-500 hover:text-mahogany-900'"
                    class="mr-8 pb-3 text-sm font-medium border-b-2 transition-colors duration-200">
                Mis Libros Prestados
                <span class="ml-2 bg-gold-500/15 text-gold-700 text-xs px-2 py-0.5 rounded-full">
                    {{ $loans->count() }}
                </span>
            </button>
            <button @click="tab = 'info'"
                    :class="tab === 'info' ? 'border-gold-500 text-gold-600' : 'border-transparent text-sepia-500 hover:text-mahogany-900'"
